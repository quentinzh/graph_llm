#!/usr/bin/env python
"""Run low-cost lambda_feat ablations for graph_llm."""
...
def build_experiment_args(base_args, exp: dict[str, float | int | str]):
    ...
    args.ckpt_dir = str(Path(base_args.ckpt_dir) / "feat_search" / tag)
    args.log_dir = str(Path(base_args.log_dir) / "feat_search" / tag)
    args.output_dir = str(Path(base_args.output_dir) / "feat_search" / tag)
...
def main() -> None:
    ...
    parser.add_argument(
        "--experiments",
        default="A0,A1,A2,A3",
        help="Comma-separated experiment names to run: A0,A1,A2,A3, or all.",
    )
    parser.add_argument(
        "--dry_run",
        action="store_true",
