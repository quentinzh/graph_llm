...
class TrainingOnlyPrototypeRetriever:
    ...
    def __init__(
        ...
        *,
        ...
        # 双 prototype 训练时同一用户画像和商品文本会跨多个 batch 重复出现，
        # 按文本缓存查询向量以避免重复调用冻结 Qwen embedding。
    ...
    def _query_text_embeddings(self, texts: list[str]) -> torch.Tensor:
        """缓存画像和商品查询向量，训练、验证、测试共享同一冻结编码器。"""
        normalized_texts = [clean_ws(text) for text in texts]
        missing_texts = list(
            dict.fromkeys(text for text in normalized_texts if text not in self._query_embeddings)
        )
        if missing_texts:
            vectors = self._encode_texts(missing_texts).detach().cpu().float()
            for text, vector in zip(missing_texts, vectors):
                self._query_embeddings[text] = vector
        return torch.stack([self._query_embeddings[text] for text in normalized_texts], dim=0)
    ...
    def retrieve_batch(
        ...
        *,
        ...
        profile_records: dict | None = None,
    ) -> PrototypeBatch:
        """为 batch 选择训练集内的 item/user prototype。

        ``excluded_row_keys`` 使训练期显式排除当前 target；``profile_records``
        允许测试阶段使用原有 train+validation 画像，但无论哪个阶段，候选索引
        都只由构造函数传入的 train_dataframe 建立。
        """
        ...
        active_profile_records = (
            self.profile_records if profile_records is None else profile_records
        )
        profile_texts = [
            profile_text_from_record(active_profile_records.get(str(raw_user)))
            for raw_user in raw_users
        ]
        profile_embeddings = self._query_text_embeddings(profile_texts)
        item_embeddings = self._query_text_embeddings(item_texts)
