...
class GraphEvidenceCIER(nn.Module):
    ...
    def _clone_kv_cache(kv_cache):
        """深拷贝 KV cache，供 beam 分支各自维护独立解码状态。"""
        if kv_cache is None:
            return None
        ...
        if isinstance(kv_cache, tuple):
            ...
            for layer in kv_cache:
                if isinstance(layer, tuple):
                    cloned_layers.append(tuple(tensor.clone() for tensor in layer))
                elif hasattr(layer, "clone"):
    ...
    def _beam_generate_single(
        ...
        """对单条样本做束搜索，返回候选 token 序列与长度归一化 logprob。"""
        num_beams = max(1, int(num_beams))
        num_return_sequences = max(1, min(int(num_return_sequences), num_beams))
        # 每条 beam: (token_ids, cumulative_logprob, kv_cache, attention_mask)
        beams = [([], 0.0, None, None)]

        for _ in range(word):
            candidates = []
            for tokens, score, kv_cache, attn_mask in beams:
                if tokens:
                    last_words = torch.tensor([[tokens[-1]]], dtype=torch.long, device=device)
                    generated_ids = torch.tensor([tokens], dtype=torch.long, device=device)
                else:
                    last_words = torch.tensor([[]], dtype=torch.long, device=device)
                    generated_ids = None

                logits, kv_next, attention_mask = self.forward(
                    input_ids=last_words,
                    profile_ids=profile_ids if kv_cache is None else None,
                    profile_mask=profile_mask if kv_cache is None else None,
                    target_item_ids=target_item_ids if kv_cache is None else None,
                ...
                topk_log_probs, topk_ids = torch.topk(log_probs, k=num_beams)

                for log_prob, token_id in zip(topk_log_probs.tolist(), topk_ids.tolist()):
                    child_tokens = tokens + [int(token_id)]
                    child_score = score + float(log_prob)
                    # 每个子分支复制独立 kv/attention，避免后续解码互相污染。
                    candidates.append(
                        (
                            child_tokens,
                            child_score,
                            self._clone_kv_cache(kv_next),
        ...
        selected = beams[:num_return_sequences]
        candidates = [tokens for tokens, _, _, _ in selected]
        normalized_logprobs = [
            (score / len(tokens)) if tokens else 0.0 for tokens, score, _, _ in selected
        ]
        return candidates, normalized_logprobs
    ...
    def beam_generate(
        ...
        """按样本串行束搜索，返回 [batch][candidate][token] 与归一化 logprob。"""
        ...
        for batch_idx in range(batch_size):
            ...
            if evidence_token_ids is not None:
                sample_evidence_ids = evidence_token_ids[batch_idx : batch_idx + 1]
            if evidence_token_mask is not None:
                sample_evidence_mask = evidence_token_mask[batch_idx : batch_idx + 1]

            candidates, logprobs = self._beam_generate_single(
                profile_ids[batch_idx : batch_idx + 1],
                profile_mask[batch_idx : batch_idx + 1],
                target_item_ids[batch_idx : batch_idx + 1],
                target_item_mask[batch_idx : batch_idx + 1],
                generation_prompt_ids=generation_prompt_ids[batch_idx : batch_idx + 1]
                if generation_prompt_ids is not None
                else None,
                generation_prompt_mask=generation_prompt_mask[batch_idx : batch_idx + 1]
                if generation_prompt_mask is not None
                else None,
                word=word,
                device=device,
                num_beams=num_beams,
