#!/usr/bin/env python
"""Run top_m_evidence ablations for graph_llm."""
...
LAMBDA_FEAT = 1e-2
TOP_VALUES = [10, 15, 20, 25, 30]
BONUS_VALUES = [0.1, 0.5]


def build_experiments() -> list[dict[str, float | int | str]]:
    experiments = []
    exp_idx = 1
    for evidence_bonus in BONUS_VALUES:
        for top_m_evidence in TOP_VALUES:
            experiments.append(
                {
                    "name": f"T{exp_idx:02d}",
...
def build_experiment_args(base_args, exp: dict[str, float | int | str]):
    ...
    args.ckpt_dir = str(Path(base_args.ckpt_dir) / "top_m_search" / tag)
    args.log_dir = str(Path(base_args.log_dir) / "top_m_search" / tag)
    args.output_dir = str(Path(base_args.output_dir) / "top_m_search" / tag)
...
def main() -> None:
    ...
    parser.add_argument(
        "--experiments",
        default=",".join(str(exp["name"]) for exp in EXPERIMENTS),
        help="Comma-separated experiment names to run: T01..T10, or all.",
    )
    parser.add_argument(
        "--dry_run",
        action="store_true",
